...
<div class="container-fluid" style="text-align: right; font-family: B Nazanin; font-size:17px">
	<form id="manage-payment">
		...
		<div class="form-group">
			<label for="" class="control-label">شماره فاکتور/دانش آموز</label>
		...
		<div class="form-group">
            <label for="" class="control-label">تاریخ</label>
            <input type="text" class="form-control text-right" id=""  name="	date_created" value="<?php echo jdate("Y/m/d H:i")?>" required readonly>
        </div>
		 <div class="form-group">
            <label for="" class="control-label">میزان شهریه(ریال)</label>
            <input type="text" class="form-control text-right" id="balance"  value="" required readonly>
        </div>
        <div class="form-group">
            <label for="" class="control-label">مبلغ پرداختی(ریال)</label>
	...
	$('#manage-payment').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				...
				if(resp.status == 1){
					alert_toast("اطلاعات با موفقیت ذخیره شد",'success')
						setTimeout(function(){
