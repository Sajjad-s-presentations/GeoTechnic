...
?>
<div class="container-fluid" style="direction: rtl; text-align: right; font-family: B Nazanin; font-size:17px">
    <form action="" id="manage-course">
        <input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
        <div class="row">
        <div class="col-lg-6 border-right">
            <h5><b>اطلاعات سالن</b></h5>
            <hr>
            <div id="msg" class="form-group"></div>
            <div class="form-group">
                <label for="" class="control-label">سالن</label>
                <input type="text" class="form-control" name="course"  value="<?php echo isset($course) ? $course :'' ?>" required>
            </div>
            <div class="form-group">
                <label for="" class="control-label">موقعیت</label>
        ...
        <div class="col-lg-6">
            <h5><b>اطلاعات شهریه</b></h5>
            <hr>
            <div class="row">
                <div class="form-group">
                    <label for="ft" class="control-label">نوع شهریه</label>
                    <input type="text" id="ft" class="form-control-sm">
                </div>
                <div class="form-group">
                    <label for="" class="control-label">مبلغ(ریال)</label>
                    <input type="number" step="any" min="0" id="amount" class="form-control-sm text-right">
                </div>
                 <div class="form-group pt-1">
                    <label for="" class="control-label">&nbsp;</label>
                    <button class="btn btn-primary btn-sm" type="button" id="add_fee">افزودن به لیست</button>
                </div>
            </div>
            <hr>
            <table class="table table-condensed" id="fee-list">
                ...
                <tfoot>
                    <tr>
                        <th colspan="2" class="text-center">جمع کل(ریال)</th>
                        <th class="text-right">
...
<script>
    ...
    $('#add_fee').click(function(){
        ...
        if(amount == '' || ft == ''){
            alert_toast("لطفا اطلاعات را وارد کنید",'warning')
            return false;
        }
    ...
    $('#manage-course').submit(function(e){
        ...
        if($('#fee-list tbody').find('[name="fid[]"]').length <= 0){
            alert_toast("حدراقل یک ردیف شهریه مورد نیاز است",'danger')
            end_load()
            return false;
        }
        $.ajax({
            ...
            success:function(resp){
                if(resp==1){
                    alert_toast("سند با موفقیت افزوده شد",'success')
                        setTimeout(function(){
                            location.reload()
                        },1000)
                }else if(resp == 2){
                $('#msg').html('<div class="alert alert-danger mx-2">سند تکراری است</div>')
                end_load()
                }   
            }
        })
