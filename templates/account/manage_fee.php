	...
	<form id="manage-fees">
		...
		<input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
		 <div class="form-group">
            <label for="" class="control-label">شماره فاکتور</label>
            <input type="text" class="form-control" name="ef_no"  value="<?php echo isset($ef_no) ? $ef_no :'' ?>" required>
        </div>
		<div class="form-group">
			<label for="" class="control-label">دانش آموز</label>
		...
		<div class="form-group">
		 ...
		 <div class="form-group">
            <label for="" class="control-label">شهریه(ریال)</label>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					$('#msg').html('<div class="alert alert-danger">شماره فاکتور تکراری است</div>')
					end_load()
				}
			}
		})
